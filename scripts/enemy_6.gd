...
func fire_cone(target: Node2D):
    ...
    for i in range(cone_shots):
        ...
        if bullet.has_method("set_velocity"):
            bullet.set_velocity(Vector2.RIGHT.rotated(base_angle + angle_offset) * bullet_speed)
        elif "velocity" in bullet:
            bullet.velocity = Vector2.RIGHT.rotated(base_angle + angle_offset) * bullet_speed
